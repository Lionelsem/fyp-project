...
const fallbackDrills = [
  {
    ...
    observations:
      "Evacuation was completed smoothly with minor delay at the west stairwell.",
    recommendations:
      "Reinforce stairwell briefing for new occupants.",
  },
  {
    id: "DR-002",
  ...
  },
  {
    ...
    observations:
      "Assembly point communication was delayed for several tenants.",
    recommendations:
      "Conduct a short refresher briefing before the next exercise.",
  },
];
...
const buildFireDrillPdf = (drill) => {
  ...
  const lines = [
    ...
    `Observations: ${
      drill?.observations || "No observations recorded."
    }`,
    `Recommendations: ${
      drill?.recommendations || "No recommendations recorded."
    }`,
    `Customer Feedback: ${
      drill?.customerComments || "No feedback added."
    }`,
  ];
...
const FireDrillReports = () => {
  ...
  /*
   * Get live fire drill data from Firebase / Firestore
   */
  const {
    fireDrills: liveFireDrills,
    loading,
  } = useCustomerLiveData(user);

  /*
   * Local state is required because we update the
   * selected drill after saving customer feedback.
   */
  ...
  const [isSavingDrillComment, setIsSavingDrillComment] =
    useState(false);

  const [drillCommentMessage, setDrillCommentMessage] =
    useState("");
  ...
  /*
   * Sync Firebase live data into local state
   */
  useEffect(() => {
    if (Array.isArray(liveFireDrills)) {
  ...
  const filteredDrills = useMemo(() => {
    ...
    return drills.filter((drill) => {
      const matchesSearch =
        !query ||
        String(drill.drillType || "")
          .toLowerCase()
          .includes(query) ||
        String(drill.buildingName || "")
          .toLowerCase()
          .includes(query) ||
        String(drill.id || "")
          .toLowerCase()
      ...
      const matchesStatus =
        !statusFilter ||
        String(drill.status || "").toLowerCase() ===
  ...
  /*
   * Update the feedback text when the selected drill changes
   */
  useEffect(() => {
    setDrillComment(
      latestDrill?.customerComments || ""
    );
  ...
  const averageTime = useMemo(() => {
    const completedDrills = drills.filter(
      (drill) => drill.totalEvacuationTime
    );
  ...
  const handleSaveDrillComment = async () => {
    if (!latestDrill?.id) {
      alert(
        "Cannot save feedback for this drill because no record is available."
      );
    ...
    try {
       ...
       */
      await addFireDrillCustomerFeedback(
        latestDrill.id,
        drillComment,
        user
      );
  ...
  /*
   * Download the latest drill report as PDF
   */
  const handleDownloadLatestDrillPdf = () => {
    if (!latestDrill) {
      alert(
        "No fire drill report is selected."
      );
    ...
    try {
      ...
      console.error(
        "Failed to export fire drill report to PDF",
        error
      );

      alert(
        "Unable to download the fire drill report PDF right now."
      );
    } finally {
        ...
        >
          <div>
            <h4 className="page-subtitle">
              Review drill schedules, outcomes,
              response times, and follow-up
              actions for your facility.
            </h4>
          </div>
      ...
      <div className="dashboard-grid">
            ...
            <div className="customer-report-summary">
              ...
              <div className="customer-report-summary-grid">
                ...
                >
                  ...
                  >
                    {formatDate(
                      latestDrill?.actualDate ||
                        latestDrill?.drillDate
                    )}
                  </strong>
                </div>
              ...
              >
                ...
                >
                  {latestDrill?.observations ||
                    "No observations recorded for this drill yet."}
                </p>
              </div>
              ...
              >
                ...
                <div className="responsive-form-actions">
                  ...
                  <button
                    ...
                    className="primary-btn"
                    onClick={
                      handleSaveDrillComment
                    }
                    disabled={
                      isSavingDrillComment ||
                      !latestDrill?.id
                    }
                  >
                    {isSavingDrillComment
        ...
        <div className="content-right">
          ...
          <div className="dashboard-card">
            ...
            >
              ...
              >
                ...
                >
                  {latestDrill?.totalEvacuationTime ||
                    "Not recorded"}
                </strong>
              </div>
              ...
              >
                ...
                >
                  {latestDrill?.recommendations ||
                    "No immediate follow-up listed."}
                </strong>
              </div>
          ...
          <div className="dashboard-card">
            ...
            >
              Contact the fire safety team if you
              need a full drill report, a briefing,
              or help with corrective actions.
            </p>
